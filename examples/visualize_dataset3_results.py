#!/usr/bin/env python3
"""
Generate comprehensive visualizations for model evaluation on Emotion Dataset 3
"""
...
def plot_overall_comparison(metrics_dict, output_dir):
    ...
    ax.set_xlabel('Model')
    ax.set_ylabel('Score (%)')
    ax.set_title('Overall Model Performance on Emotion Dataset 3 Test Set')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "dataset3_overall_comparison.png", bbox_inches='tight')
    plt.savefig(output_dir / "dataset3_overall_comparison.pdf", bbox_inches='tight')
    plt.close()

    print("Created: dataset3_overall_comparison.png/.pdf")
...
def plot_per_emotion_f1(metrics_dict, output_dir):
    ...
    ax.set_title('Per-Emotion F1-Score Comparison - Dataset 3 Test Set')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "dataset3_per_emotion_f1.png", bbox_inches='tight')
    plt.savefig(output_dir / "dataset3_per_emotion_f1.pdf", bbox_inches='tight')
    plt.close()

    print("Created: dataset3_per_emotion_f1.png/.pdf")
...
def plot_confusion_matrices(metrics_dict, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "dataset3_confusion_matrices.png", bbox_inches='tight')
    plt.savefig(output_dir / "dataset3_confusion_matrices.pdf", bbox_inches='tight')
    plt.close()

    print("Created: dataset3_confusion_matrices.png/.pdf")
...
def plot_precision_recall_comparison(metrics_dict, output_dir):
    ...
    ax1.set_title('Per-Emotion Precision Comparison')
    ax1.set_xticks(x)
    ...
    ax2.set_title('Per-Emotion Recall Comparison')
    ax2.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "dataset3_precision_recall.png", bbox_inches='tight')
    plt.savefig(output_dir / "dataset3_precision_recall.pdf", bbox_inches='tight')
    plt.close()

    print("Created: dataset3_precision_recall.png/.pdf")


def plot_class_support(metrics_dict, output_dir):
    """Show class distribution in test set"""
    emotions = ['angry', 'disgusted', 'fearful', 'happy', 'sad', 'surprised', 'neutral']

    # Get support from first model (should be same for all)
    first_model = list(metrics_dict.keys())[0]
    support = [metrics_dict[first_model]['per_class'][e]['support'] for e in emotions]
    total = sum(support)
    percentages = [s/total*100 for s in support]

    fig, ax = plt.subplots(figsize=(10, 6))

    bars = ax.bar(emotions, support, color='#4ECDC4', alpha=0.8)
    ...
    ax.set_title(f'Class Distribution in Dataset 3 Test Set (Total: {total})')
    ax.set_xticklabels([e.title() for e in emotions], rotation=15, ha='right')
    ax.grid(axis='y', alpha=0.3)

    plt.tight_layout()
    plt.savefig(output_dir / "dataset3_class_distribution.png", bbox_inches='tight')
    plt.savefig(output_dir / "dataset3_class_distribution.pdf", bbox_inches='tight')
    plt.close()

    print("Created: dataset3_class_distribution.png/.pdf")
...
def generate_metrics_table(metrics_dict, output_dir):
    ...
    output_path = output_dir / "dataset3_detailed_metrics.csv"
    df.to_csv(output_path, index=False)
    print(f"Created: dataset3_detailed_metrics.csv")
...
def print_summary_report(metrics_dict):
    ...
    print("\n" + "="*80)
    print("EMOTION DATASET 3 - MODEL EVALUATION SUMMARY")
    print("="*80)
...
def main():
    ...
    output_dir = Path(__file__).parent.parent / "results" / "dataset3_evaluation"

    # Create output directory
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print("VISUALIZING EMOTION DATASET 3 MODEL EVALUATION RESULTS")
    print("="*80)
    ...
    if not results_file.exists():
        ...
        print("  python examples/evaluate_models_dataset3.py")
        return
    ...
    plot_class_support(metrics_dict, output_dir)
