#!/usr/bin/env python3
"""
Ensemble approach for Faces_Dataset: Use Custom-ResNet18 as base, override with DeepFace+FER agreement
"""
...
def plot_accuracy_comparison(metrics_list, output_dir):
    ...
    ax.set_xlabel('Model')
    ax.set_ylabel('Score (%)')
    ax.set_title('Performance Comparison: Individual Models vs. Ensemble (Faces_Dataset)\n' +
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "faces_ensemble_accuracy_comparison.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_ensemble_accuracy_comparison.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_ensemble_accuracy_comparison.png/.pdf")
...
def plot_per_emotion_comparison(metrics_list, output_dir):
    ...
    ax.set_title('Per-Emotion F1-Score: Custom-ResNet18 vs. Ensemble (Faces_Dataset)')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "faces_ensemble_per_emotion_f1.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_ensemble_per_emotion_f1.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_ensemble_per_emotion_f1.png/.pdf")
...
def plot_confusion_matrix(metrics, output_dir):
    ...
    ax.set_title(f'Ensemble Confusion Matrix (Faces_Dataset)\nAccuracy: {metrics["accuracy"]*100:.1f}%',
                fontsize=14)
    ax.set_ylabel('True Emotion')
    ax.set_xlabel('Predicted Emotion')

    plt.tight_layout()
    plt.savefig(output_dir / "faces_ensemble_confusion_matrix.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_ensemble_confusion_matrix.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_ensemble_confusion_matrix.png/.pdf")
...
def plot_override_impact(override_analysis, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "faces_ensemble_override_impact.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_ensemble_override_impact.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_ensemble_override_impact.png/.pdf")
...
def plot_disagreement_performance(disagreement_analysis, overall_metrics, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "faces_disagreement_performance.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_disagreement_performance.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_disagreement_performance.png/.pdf")
...
def plot_agreement_patterns(df, output_dir):
    ...
    ax.set_title('Model Agreement Patterns (Faces_Dataset)')
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=20, ha='right')
    ax.grid(axis='y', alpha=0.3)

    plt.tight_layout()
    plt.savefig(output_dir / "faces_ensemble_agreement_patterns.png", bbox_inches='tight')
    plt.savefig(output_dir / "faces_ensemble_agreement_patterns.pdf", bbox_inches='tight')
    plt.close()

    print("Created: faces_ensemble_agreement_patterns.png/.pdf")
...
def generate_detailed_report(metrics_list, override_analysis, patterns, disagreement_analysis, output_dir):
    ...
    report_lines.append("ENSEMBLE EVALUATION REPORT - FACES_DATASET")
    ...
    # Save report
    report_path = output_dir / "faces_ensemble_evaluation_report.txt"
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))

    print(f"Created: faces_ensemble_evaluation_report.txt")
...
def main():
    # Configuration
    results_file = Path(__file__).parent.parent / "data" / "processed" / "emotion_results_comparison.csv"
    output_dir = Path(__file__).parent.parent / "results" / "faces_ensemble_evaluation"

    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print("ENSEMBLE APPROACH FOR FACES_DATASET")
    print("Strategy: Custom-ResNet18 + Override when DeepFace & FER Agree")
    print("="*80)

    # Check if results exist
    if not results_file.exists():
        print(f"\nERROR: Results file not found: {results_file}")
        print("Please run: python examples/detect_emotions.py")
        return
